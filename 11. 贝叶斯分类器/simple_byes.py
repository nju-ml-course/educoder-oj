    ...
    def fit(self, feature, label):
        ...
        def store_prop():
            ...
            for labelItem in self.label_indexes.keys():
                # 拉普拉斯修正
                self.label_prob[labelItem] = (len(self.label_indexes[labelItem]) + 1) / m
            # ------------------------------
        ...
        for i, f in enumerate(feature):
            ...
            for label, label_prob in self.label_prob.items():
                ...
                for j in range(len(feature[0])):
                    prob[ii] *= self.condition_prob[label][j][f[j]] if f[j] in self.condition_prob[label][
                        j].keys() else 0
                ii += 1
            # 取概率最大的类别作为结果
